...
std::vector<Vector3I> voxel_traversal(const Vector3R& ray_start, const Vector3R& ray_end, const double bin_size) {
    ...
    Vector3I current_voxel(std::floor(ray_start[0] / bin_size), std::floor(ray_start[1] / bin_size),
                           std::floor(ray_start[2] / bin_size));

    // The id of the last voxel hit by the ray.
    // TODO: what happens if the end point is on a border?
    Vector3I last_voxel(std::floor(ray_end[0] / bin_size), std::floor(ray_end[1] / bin_size),
    ...
    // (tMaxX, tMaxY, tMaxZ).
    double next_voxel_boundary_x = (current_voxel[0] + stepX) * bin_size;   // correct
    double next_voxel_boundary_y = (current_voxel[1] + stepY) * bin_size;   // correct
    double next_voxel_boundary_z = (current_voxel[2] + stepZ) * bin_size;   // correct
...
double find_ray_voxel_intersection_parameter(const Vector3R& ray_start, const Vector3R& ray_end,
    ...
    double t = ((current_voxel[ax] + dir) * bin_size - ray_start[ax]) / (ray_end[ax] - ray_start[ax]);
    return t;
}
...
//     std::cout << "> " << i << std::endl;
//   }
//   std::cout << "Total number of traversed voxels: " << ids.size() <<
//   std::endl; Vector3I current_voxel = ids(0); for (int i =1; i < ids.size();
//   i++) {
