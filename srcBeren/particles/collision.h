...
class BinaryCollider {
   ...
   public:
    void collide_same_sort_binary(Species &species, const double dt);
...
class BinaryCollisionSameType {
   public:
    ...
    bool canCollide() {
        return (v.size() > 1) && (event < maxEvent - 1);
    }
...
class BinaryCollisionDiffType {
   public:
    ...
    bool canCollide() {
        return (v1.size() == v2.size() || (v1.size() > 1 && v2.size() > 0)) && (event < maxEvent - 1);
    }
    std::pair<int, int> get_pair();
