...
//     double rz = 15; //0.5 * cellSize.z() * domain.num_cells(Z);
//     double rr = 10; // 30 * cellSize.x();
...
void Simulation::make_all() {
    ...
    init(mesh);
    set_coils(mesh.fieldBInit, domain, parameters);
    ...
    BinaryCollider collider(parameters.get_double("n0"));
    ...
    for (auto timestep = startTimeStep + 1; timestep <= lastTimestep;
        ...
        if(parameters.get_string("Collider") == "BinaryCollider") {
        ...
        damping_fields(mesh.fieldEn, mesh.fieldB, mesh.fieldBInit,
                                    domain, parameters);
