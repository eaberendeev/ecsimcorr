...
void Writer::write_particles2D(int timestep){ 
	...
	for( auto& sp : _species){
		...
		for (auto coordX : diagData.params.sliceFieldsPlaneX){
			write_array2D_planeX(sp.densityOnGrid, coordX, fnameDense, timestep);
      ...
      write_fields2D_planeX(sp.currentOnGrid, coordX, fnameJ, timestep);
		...
		for (auto coordY : diagData.params.sliceFieldsPlaneX){
			write_array2D_planeY(sp.densityOnGrid, coordY, fnameDense, timestep);
      ...
      write_fields2D_planeY(sp.currentOnGrid, coordY, fnameJ, timestep);
		...
		for (auto coordZ : diagData.params.sliceFieldsPlaneZ){
			write_array2D_planeZ(sp.densityOnGrid, coordZ, fnameDense, timestep);
...
void Writer::write_array2D_planeX(const Array3D<double>& data, double coordX, const std::string& fname, const int& timestep){
...
void Writer::write_array2D_planeZ(const Array3D<double>& data, double coordZ, const std::string& fname, const int& timestep){
    char filenameCh[100];
    float info;    
...
void Writer::write_array2D_planeY(const Array3D<double>& data, double coordY, const std::string& fname, const int& timestep){
    char filenameCh[100];
    float info;    
