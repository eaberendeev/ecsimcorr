...
void Simulation::make_all() {
    ...
    init(mesh);
    RandomGenerator gen;
    // for (auto i = 0; i < mesh.fieldE.capacity(); ++i) {
    //     mesh.fieldE(i) = 0.0;//gen.Uniform01();
    // }
    // for (auto i = 0; i < mesh.fieldE.capacity(); ++i) {
    //     mesh.fieldB(i) = 0.0;   // gen.Uniform01();
    //     mesh.fieldB(i) = 0.0;   // gen.Uniform01();
    // }
    // mesh.fieldE(2, 2, 2, X) = 100;
    // mesh.fieldE(2, 2, 3, X) = 50;
    // mesh.fieldE(2, 3, 2, X) = 50;
    // mesh.fieldE(2, 3, 3, X) = 50;
    auto size = mesh.fieldE.size();
    for (auto i = 0; i < size.x(); ++i) {
        for (auto j = 0; j < size.y(); ++j) {
            for (auto k = 0; k < size.z(); ++k) {
                //mesh.fieldE(i, j, k, Dim::Y) = 0.0; //0.001;
                //mesh.fieldB(i, j, k, Dim::Z) = 0.2; //*(1-0.8*(j-0.5 - 3.4));
            }
        }
    }
    set_coils(mesh.fieldB, domain, parameters);
    std::ofstream fout("track.txt");
    std::ofstream fb("B.txt");

    for (auto k = 0; k < size.z(); ++k) {
        int i = 32;
        int j = 32;
        fb << mesh.fieldB(i, j, k, Dim::Z) <<"\n"; //*(1-0.8*(j-0.5 - 3.4));
    }

    for (auto &sp : species) {
        if(sp.name() == "Ions") continue;
        // double3 mid_point(0,0,0);
        // double3 cellSize = domain.cell_size();
        // Node node(mid_point, cellSize);
        // std::cout <<node.r << " " <<node.g << " " <<node.g05 << "\n";
        // ShapeK shape, new_shape;
        // sp.fill_shape(node, shape, false);
        // sp.fill_shape(node, new_shape, true);
        // std::cout << sp.interpolateB(mesh.fieldB, node, shape, new_shape) << "\n";


        Particle p; 
        p.coord = double3(32, 32+27, 344/2.);
        p.velocity = double3(0.02, 0., 0.018);
        //std::cout << get_fieldE_in_pos(mesh.fieldE, p.coord, domain) << "\n";
    ...
    for (auto &sp : species) {
        ...
        for (auto pk = 0; pk < sp.size(); ++pk) {
            ...
            for (auto &particle : sp.particlesData(pk)) {
                double3 coord = particle.coord;
                fout << coord << "\n";
            }
        }

        sp.push_Chen(mesh.fieldE, mesh.fieldB, 1);
    }
    //todo: add Bz track and Ey x Bz test
    //todo: add energy conservation
    //todo: add gradient B test
    fout.close();
    fb.close();
    exit(0);
    ...
    for (auto timestep = startTimeStep + 1; timestep <= lastTimestep;
     ...
     //   inject_particles(timestep);
        ...
        for (auto &sp : species) {
            ...
            sp.push_Chen(mesh.fieldE, mesh.fieldB, dt);
        }
     //   exit(0);
