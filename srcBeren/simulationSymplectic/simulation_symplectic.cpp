...
void SimulationSymplectic::make_step([[maybe_unused]] const int timestep) {
    ...
    std::cout << "Energy JE: " << dt * energyJeEn << " " << mesh.calc_energy_field(fieldB) + energy - energy0 << " "
              << ek2 - ek << std::endl;
        ...
        for (int j = 0; j < mesh.chargeDensity.size().y()) {
            for (int k = 0; k < mesh.chargeDensity.size().z()) {
                if (mesh.chargeDensity(i, j, k, 0) != 0)
                    std::cout << i << " " << j << " " << k << " " << mesh.chargeDensity(i, j, k, 0) << "\n";
            }
        }
    }
    for (int i = 0; i < mesh.chargeDensity.size().x()) {
        for (int j = 0; j < mesh.chargeDensity.size().y()) {
            for (int k = 0; k < mesh.chargeDensity.size().z()) {
                if (mesh.chargeDensity(i, j, k, 0) != 0)
                    std::cout << i << " " << j << " " << k << " " << mesh.chargeDensityOld(i, j, k, 0) << "\n";
            }
        }
    }
...
void SimulationSymplectic::make_diagnostic(const int timestep) {
    ...
    write_fields_to_recovery(fieldE, fieldB, timestep, parameters.get_int("RecoveryInterval"));
    ...
    std::vector<std::pair<const Field3d &, std::string>> fields = {{fieldE, pathToField + "FieldE"},
...
void SimulationSymplectic::diagnostic_energy(Diagnostics &diagnostic) {
    ...
    diagnostic.addEnergy("energyFieldBFull", mesh.calc_energy_field(fieldBFull));
    ...
    const double dt = parameters.get_double("Dt");
    diagnostic.addEnergy("JpEnergy", 0.5 * fieldJ.data().dot(fieldE.data() + fieldE_initial.data()));
    diagnostic.addEnergy("JpEnergyConservation", diagnostic.energy["JpEnergy"] - field_energy);
