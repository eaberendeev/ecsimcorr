...
std::pair<double, double> interpolate_magnetic_fields(int cell_idx, const auto& weights, auto& fieldB) {
    ...
    if constexpr (DIR == Direction::X) {
        ...
        double Bz =
            (fieldB(cell_idx, j_idx, k_idx, Z) * w1.weights[0] + fieldB(cell_idx, j_idx, k_idx + 1, Z) * w1.weights[1]);
        double By =
            (fieldB(cell_idx, j_idx, k_idx, Y) * w2.weights[0] + fieldB(cell_idx, j_idx + 1, k_idx, Y) * w2.weights[1]);
        return {Bz, By};
        ...
        double Bz =
            (fieldB(i_idx, cell_idx, k_idx, Z) * w1.weights[0] + fieldB(i_idx, cell_idx, k_idx + 1, Z) * w1.weights[1]);
        double Bx =
            (fieldB(i_idx, cell_idx, k_idx, X) * w2.weights[0] + fieldB(i_idx + 1, cell_idx, k_idx, X) * w2.weights[1]);
        return {Bz, Bx};
        ...
        double By =
            (fieldB(i_idx, j_idx, cell_idx, Y) * w1.weights[0] + fieldB(i_idx, j_idx + 1, cell_idx, Y) * w1.weights[1]);
        double Bx =
            (fieldB(i_idx, j_idx, cell_idx, X) * w2.weights[0] + fieldB(i_idx + 1, j_idx, cell_idx, X) * w2.weights[1]);
        return {By, Bx};
    }
}

template <Direction DIR>
void update_current_density(int cell_idx, const auto& weights, double charge, double path, double dt, auto& fieldJ) {
    const auto& [w1, w2] = weights;
...
#pragma omp atomic update
                fieldJ(cell_idx, j_idx, k_idx, X) += weight * charge * path / dt;
                ...
                const double weight = w1.weights[n1] * w2.weights[n2];
#pragma omp atomic update
                fieldJ(i_idx, cell_idx, k_idx, Y) += weight * charge * path / dt;
                ...
                const double weight = w1.weights[n1] * w2.weights[n2];
#pragma omp atomic update
                fieldJ(i_idx, j_idx, cell_idx, Z) += weight * charge * path / dt;
            }
        }
    }
}
...
template <Direction DIR>
void update_velocities(auto& velocity, double charge, double mass, double total_B1, double total_B2) {
    if constexpr (DIR == Direction::X) {
...
template <Direction DIR>
void SimulationSymplectic::move_particles_impl(ParticlesArray& species, const double dt) {
...
#pragma omp parallel for schedule(dynamic, 64)
    for (auto pk = 0; pk < species->size(); ++pk) {
        for (auto& particle : species->particlesData(pk)) {
            ...
            const int initial_cell = static_cast<int>((start_pos / cell_size) + GHOST_CELLS);
            const int final_cell = static_cast<int>((end_pos / cell_size) + GHOST_CELLS);
            const int cell_diff = final_cell - initial_cell;

            // Вычисляем веса для двух других направлений
            const auto weights = compute_weights_for_direction<DIR>(particle, cell_sizes);

            const auto process_cell = [&](int cell_idx, double path) {
                // Получаем магнитные поля для двух перпендикулярных направлений
                auto [B_field1, B_field2] = interpolate_magnetic_fields<DIR>(cell_idx, weights, fieldB);

                // Обновляем ток в текущем направлении
                update_current_density<DIR>(cell_idx, weights, charge, path, dt, fieldJ);
            ...
            if (cell_diff == 0) {
                ...
                const double boundary_pos = cell_size * (initial_cell + (cell_diff > 0) - GHOST_CELLS);
            ...
            update_velocities<DIR>(particle.velocity, charge, mass, total_B1, total_B2);
...
const std::map<Direction, DirectionConfig> dir_config = {
    {Direction::X, {Z, Y, Y, Z}}, {Direction::Y, {Z, X, Z, X}}, {Direction::Z, {Y, X, X, Y}}};
...
void SimulationSymplectic::move_particles_impl(ParticlesArray& species, const double dt) {
    ...
    const auto normalided_pos = [&](double pos, int dir) { return (pos / cell_sizes[dir]); }
#pragma omp parallel for schedule(dynamic, 64)
    for (auto pk = 0; pk < species->size(); ++pk) {
        for (auto& particle : species->particlesData(pk)) {
            // Получаем координаты и скорости в зависимости от направления
            ...
            const auto w1 = compute_weights(normalided_pos(coord(cfg.b1_comp) + GHOST_CELLS, cfg.b1_comp), 0.0);
            const auto w2 = compute_weights(normalided_pos(coord(cfg.b2_comp) + GHOST_CELLS, cfg.b2_comp), 0.0);
            const auto process_cell = [&](int cell_idx, double path) {
                // Получаем магнитные поля для двух перпендикулярных направлений
                auto [B_field1, B_field2] = interpolate_magnetic_fields<DIR>(cell_idx, weights, fieldB);
