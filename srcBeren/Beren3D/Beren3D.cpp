...
void get_charge_density(Mesh &mesh, Field3d &field, const std::vector<ParticlesArray> &species );
...
///// Make particles
	...
	for(auto timestep = StartTimeStep + 1; timestep <= 2000000; ++timestep){
		...
		for( auto &sp  : species){
			//reserve_Lmat(mesh, sp);
			//sp.move(0.5*Dt); //  +++ x_n -> x_{n+1/2}
			sp.move_and_calc_current(0.5*Dt); //  +++ x_n -> x_{n+1/2}
			sp.predict_current(mesh.fieldB,mesh.fieldJp); // +++ get J(x_{n+1/2},v_n)_predict 
		...
		mesh.predictE(); // --- solve A*E'_{n+1}=f(E_n, B_n, J(x_{n+1/2})). mesh consist En, En+1_predict
		globalTimer.finish("FieldsPredict");

		globalTimer.start("particles2");
		energyP = energyPn = 0.;
		for( auto &sp  : species){
			energyP += sp.get_kinetic_energy() ;  // / sp.mpw(0);
//
			sp.predict_velocity(mesh); // +++ get v'_{n+1} from v_{n} and E'_{n+1}
		 	sp.update_cells();

		 	//sp.move(0.5*Dt); // +++ x_{n+1/2} -> x_{n+1}
			sp.move_and_calc_current(0.5*Dt); //  +++ x_n -> x_{n+1/2}

			//sp.calc_Esirkepov_current(Dt, mesh.fieldJe);
			sp.update_cells();
			//sp.density_on_grid_update();
		...
		mesh.correctE(); // ---- get E_{n+1} from E_n and J_e. mesh En changed to En+1_final 
		...
		//auto divJ = mesh.divE*(mesh.fieldJp.data() + (1/Dt)*mesh.Lmat2*(mesh.fieldE.data() + mesh.fieldEp.data()));
