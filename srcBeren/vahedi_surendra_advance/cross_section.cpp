...
constexpr double kIonizationConsistency =
    (THRESHOLD_IONIZATION > E_ion) ? (THRESHOLD_IONIZATION - E_ion) : (E_ion - THRESHOLD_IONIZATION);
...
double ColliderWithNeutrals::Sigma_e(double energy) const {
    if (energy > THRESHOLD_IONIZATION) {
        double s1 = CROSS_SECTION_B * exp(-CROSS_SECTION_C * (energy / THRESHOLD_IONIZATION - 1.));
        double s2 = CROSS_SECTION_A * log(energy / THRESHOLD_IONIZATION) / (energy * THRESHOLD_IONIZATION);
        return s2 * (1 - s1) * dpd0;
    } else {
        return 0.;
    }
...
double ColliderWithNeutrals::Sigma_cx(double energy) const {
    ...
    Scx /= (1 + CROSS_SECTION_A3 * energy + CROSS_SECTION_A4 * pow(energy, 3.5) + CROSS_SECTION_A5 * pow(energy, 5.4));
    return Scx * dpd0;
}
...
double ColliderWithNeutrals::Sigma_p(double energy) const {
    if (energy > THRESHOLD_IONIZATION) {
        energy = energy * SGS::MC2 / CROSS_SECTION_amuH;
        double Sp = 1e-16;
        double Sp_left = exp(-CROSS_SECTION_B2 / energy) * log(1. + CROSS_SECTION_B3 * energy) / energy;
        double Sp_right = CROSS_SECTION_B4 * exp(-CROSS_SECTION_B5 * energy) /
                          (pow(energy, CROSS_SECTION_B6) + CROSS_SECTION_B7 * pow(energy, CROSS_SECTION_B8));
